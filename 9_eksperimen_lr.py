...
"""### 9.3 5-Fold Cross-Validation — Two-Stage Logistik Regression
...
for train_idx, test_idx in skf.split(X_all, y_all):
    ...
    lr1_cv = LogisticRegression(C=1.0, class_weight='balanced', solver='saga', max_iter=1000, random_state=42)
    lr1_cv.fit(X_tr_tfidf_cv, y1_tr)
    p1_cv = lr1_cv.predict(X_te_tfidf_cv)

    # Stage 2
    mask_tr = (y_tr != 'positive').values
    mask_te = (y_te != 'positive').values
    y2_tr = (y_tr.values[mask_tr] == 'negative').astype(int)
    lr2_cv = LogisticRegression(C=1.0, class_weight='balanced', solver='saga', max_iter=1000, random_state=42)
    lr2_cv.fit(X_tr_tfidf_cv[mask_tr], y2_tr)
    p2_cv = lr2_cv.predict(X_te_tfidf_cv[mask_te])
...
plt.ylabel('Score')
plt.title('5-Fold CV Results — Two-Stage Logistic Regression', fontsize=14, fontweight='bold')
